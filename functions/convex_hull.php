...
class ConvexHull 
{
    ...
    {
        foreach($pofloats as $key => $value){
			$pofloats[$key] = explode(',',$value);	
		}
    ...
    public function getHullPoints() 
    {
		...
		$hulls = $this->hullPoints;
		foreach($hulls as $key => $hull){
			$hulls[$key] = implode(', ',$hull);
		}
		
		//echo '<hr /><pre>',print_r($hulls),'</pre>';
